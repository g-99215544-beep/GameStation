...
- `package.json` is gitignored — never commit it. There is no `node_modules`; `npx playwright` resolves from the npx cache.
- **Known-failing baseline.** `tests/station-count.spec.js:83` ("saving fewer stations regenerates group orders and preserves roster") already fails on `origin/main`, before any cannon work: it expects a group `order` of length 3 after saving 3 stations and gets 6. It is unrelated to cannons and **must not be fixed as part of this plan**. The whole-suite baseline is therefore **55 passed, 1 failed**. Any other failure is a real regression.
...
Expected: identical to Step 1 — `worksheet-images` fully passing, and `station-count` at its known baseline of 4 passed / 1 failed (`station-count.spec.js:83`, failing before this plan started and out of scope here). No behaviour has changed.
...
Expected: every existing spec still at baseline — the only failure is the known `tests/station-count.spec.js:83`.
...
Expected: every suite at baseline — all unit tests pass, and the only Playwright failure is the known `tests/station-count.spec.js:83`.
